// server.js - Integrated Express server for BaptistCare website & Boomi Chatbot Proxy
...
// Serve static front-end assets (index.html, contact.html, style.css, chat.js, etc.)
...
// Serve chat widget (fallback or admin info)
app.get('/chat-info', (req, res) => {
    res.json({
        message: "Southern Cross Care Chatbot Proxy is active",
        boomi_endpoints: {
            chat: BOOMI_ENDPOINT,
            session: BOOMI_SESSION_ENDPOINT
        }
...
// Start server
const PORT = process.env.PORT || 3000; // default to 3000 for BaptistCare
...
app.listen(PORT, HOST, () => {
    console.log(`🚀 Starting BaptistCare Boomi Chatbot Proxy on ${HOST}:${PORT}`);
